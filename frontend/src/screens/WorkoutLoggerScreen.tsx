...
const MuscleGroupPill = React.memo(function MuscleGroupPill({ label, selected, onPress }: any) {
  return (
  <TouchableOpacity
    onPress={onPress}
    style={{
...
const ExerciseCard = React.memo(function ExerciseCard({ exercise, onPress }: any) {
  return (
  <TouchableOpacity
    onPress={onPress}
    style={{
    ...
    try {
      ...
      const exercises = Object.entries(activeSets).map(([id, sets]) => {
        ...
        return {
          ...
          sets: (sets as any[]).filter(s => s.completed).map(s => ({
