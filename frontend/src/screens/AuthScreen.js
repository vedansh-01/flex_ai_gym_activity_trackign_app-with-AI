...
// This screen is shown AFTER registration. It is non-dismissible —
// the user MUST verify their email before they can do anything else.
function OTPScreen({ email, onVerified, onCancel }) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        onVerified(data.token, data);
      } else {
        Alert.alert('Verification Failed', data.message || 'Invalid or expired code. Try again.');
      }
    } catch {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const data = await response.json();
      Alert.alert('Code Sent', data.message || 'A new verification code has been sent to your email.');
    ...
    try {
      ...
      if (response.ok || data.requiresVerification) {
        if (data.requiresVerification) {
          // Force user to the OTP screen — no way around it
          setStep('verify');
        } else {
          await storage.setItem('userToken', data.token);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.footnote}>🔒 Secured with JWT · Email verified</Text>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
}
