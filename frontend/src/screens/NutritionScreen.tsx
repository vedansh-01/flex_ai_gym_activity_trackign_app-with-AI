...
// ─── Calorie Ring ─────────────────────────────────────────────────────────────
const CalorieRing = React.memo(function CalorieRing({ consumed, goal }: any) {
...
// ─── Macro Bar ────────────────────────────────────────────────────────────────
const MacroBar = React.memo(function MacroBar({ label, current, goal, color }: any) {
...
// ─── Food Item Row ────────────────────────────────────────────────────────────
const FoodItem = React.memo(function FoodItem({ food, onDelete }: any) {
  return (
...
// ─── Meal Section Card ────────────────────────────────────────────────────────
const MealSection = React.memo(function MealSection({ mealKey, meal, onAddFood, onDeleteFood }: any) {
...
export default function NutritionScreen() {
  ...
  const [profile,  setProfile]    = useState<any>(null); // user profile for goal calculation
  ...
  useEffect(() => {
    ...
    loadFoodDatabase().then(foods => setAllFoods(foods as any[]));
  }, []);
  ...
  const loadWater = async () => {
    ...
    } catch (e) { console.error('Water load error:', e); }
  };
  ...
  const handleAddWater = async (amount) => {
    ...
    } catch (e) { console.error('Water add error:', e); }
  };
  ...
  const handleDeleteWater = async (id) => {
    try {
      ...
      if (res.ok) await loadWater();
    } catch (e) { console.error('Water delete error:', e); }
  };
            ...
            <View style={s.waterBox}>
              <View style={s.waterHeader}>
                <View>
                  ...
                  <Text style={{ fontSize: 11, color: '#A1A1AA', marginTop: 4 }}>Personalized daily hydration target</Text>
                </View>
              ...
              <View style={s.waterActions}>
                {[
                  ...
                  <TouchableOpacity key={act.label} style={s.waterBtn} onPress={() => handleAddWater(act.val)}>
                    <Ionicons name={act.icon as any} size={14} color="#60A5FA" />
...
const s = StyleSheet.create({
  ...
  eyebrow:     { color: theme.colors.textSecondary, fontSize:11, letterSpacing:1.5, textTransform:'uppercase' },
  title:       { color: theme.colors.text, fontSize:24, fontWeight:'800', marginTop:2 },
  dateBadge:   { flexDirection:'row', alignItems:'center', gap:6, backgroundColor: theme.colors.surface, borderRadius: 10, paddingHorizontal:12, paddingVertical:8, borderWidth:1, borderColor: theme.colors.border },
  dateTxt:     { color: theme.colors.primary, fontSize:13, fontWeight:'700' },
  // Summary
  summaryCard: { margin:16, backgroundColor: theme.colors.surface, borderRadius:24, padding:20, borderWidth:1, borderColor: theme.colors.border },
  ringWrap:    { alignItems:'center', justifyContent:'center', marginBottom:20 },
  ringCenter:  { position:'absolute', alignItems:'center' },
  ringCal:     { color: theme.colors.text, fontSize:32, fontWeight:'900' },
  ringLabel:   { color: theme.colors.textTertiary, fontSize:11, marginTop:2 },
  ringDivider: { width:30, height:1, backgroundColor: theme.colors.border, marginVertical:6 },
  ringRemain:  { color: theme.colors.primary, fontSize:12, fontWeight:'700' },
  ...
  macroValue:  { color: theme.colors.text, fontSize:12, fontWeight:'700' },
  macroGoal:   { color: theme.colors.textTertiary, fontWeight:'400' },
  macroTrack:  { height:6, backgroundColor: theme.colors.border, borderRadius:3, overflow:'hidden' },
  macroFill:   { height:6, borderRadius:3 },
  statRow:     { flexDirection:'row', marginTop:4 },
  statCell:    { flex:1, alignItems:'center' },
  statVal:     { fontSize:14, fontWeight:'800' },
  statLabel:   { color: theme.colors.textTertiary, fontSize:10, marginTop:2 },
  // Sections
  section:       { paddingHorizontal:16, marginTop:8 },
  sectionHeader: { color: theme.colors.textSecondary, fontSize:11, fontWeight:'700', letterSpacing:1.5, textTransform:'uppercase', marginBottom:12 },
  // Meal cards
  mealCard:    { backgroundColor: theme.colors.surface, borderRadius:18, borderWidth:1, borderColor: theme.colors.border, marginBottom:10, overflow:'hidden' },
  ...
  mealLabel:   { color: theme.colors.text, fontSize:15, fontWeight:'700' },
  mealSub:     { color: theme.colors.textTertiary, fontSize:11, marginTop:2 },
  ...
  addBtnTxt:   { color: theme.colors.text, fontWeight: '800', fontSize:13 },
  // Food item
  foodRow:      { flexDirection:'row', alignItems:'flex-start', paddingVertical:10, borderTopWidth:1, borderTopColor: theme.colors.border },
  foodName:     { color: theme.colors.text, fontSize:14, fontWeight:'600' },
  foodServing:  { color: theme.colors.textTertiary, fontSize:11, marginTop:2 },
  ...
  foodCal:      { color: theme.colors.text, fontSize:16, fontWeight:'800' },
  foodCalLabel: { color: theme.colors.textTertiary, fontSize:10 },
  // History
  historyCard:  { backgroundColor: theme.colors.surface, borderRadius:18, borderWidth:1, borderColor: theme.colors.border, padding:14, marginBottom:10, flexDirection:'row', alignItems:'center', gap:12 },
  historyDot:   { width:10, height:10, borderRadius:5 },
  historyCat:   { color: theme.colors.primary, fontSize:10, fontWeight:'700', textTransform:'uppercase' },
  historyName:  { color: theme.colors.text, fontSize:15, fontWeight:'700', marginVertical:2 },
  historySub:   { color: theme.colors.textTertiary, fontSize:11 },
  emptyCard:    { backgroundColor: theme.colors.surface, borderRadius:16, padding:20, alignItems:'center', borderWidth:1, borderColor: theme.colors.border },
  emptyCardTxt: { color: '#3F3F46', fontSize:13 },
  // Water box
  waterBox:     { backgroundColor: theme.colors.surface, borderRadius:20, padding:18, borderWidth:1, borderColor: theme.colors.border, marginBottom:12 },
  waterHeader:  { flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:12 },
  waterGoalTxt: { color: theme.colors.text, fontSize:20, fontWeight:'800' },
  waterTrack:   { height:8, backgroundColor: theme.colors.border, borderRadius:4, overflow:'hidden', marginBottom:16 },
  waterFill:    { height:8, backgroundColor: theme.colors.info, borderRadius:4 },
  waterActions: { flexDirection:'row', gap:8, marginBottom:12 },
  waterBtn:     { flex:1, flexDirection:'row', alignItems:'center', justifyContent:'center', gap:6, paddingVertical:10, backgroundColor: theme.colors.primarySoft, borderRadius:10, borderWidth:1, borderColor: theme.colors.border },
  waterBtnTxt:  { color: theme.colors.info, fontSize:12, fontWeight:'700' },
  waterLogs:    { borderTopWidth:1, borderTopColor: theme.colors.border, paddingTop:12 },
  waterLogRow:  { flexDirection:'row', alignItems:'center', gap:10, marginBottom:8 },
  waterLogAmt:  { color: theme.colors.text, fontSize:13, fontWeight:'600', flex:1 },
  waterLogTime: { color: theme.colors.textTertiary, fontSize:11 },
  // Supplements
  suppBox:      { backgroundColor: theme.colors.surface, borderRadius:20, padding:18, borderWidth:1, borderColor: theme.colors.border },
  ...
  suppName:     { color: theme.colors.text, fontSize:14, fontWeight:'700' },
  suppDose:     { color: theme.colors.textTertiary, fontSize:11, marginTop:2 },
  suppCheck:    { padding:4 },
  suppEmpty:    { color: '#3F3F46', fontSize:12, textAlign:'center', flex:1 },
  // Modal
  overlay:      { ...StyleSheet.absoluteFillObject, backgroundColor:'rgba(0,0,0,0.65)' },
  sheet:        { position:'absolute', bottom:0, left:0, right:0, backgroundColor: theme.colors.surface, borderTopLeftRadius:28, borderTopRightRadius:28, padding:20, maxHeight:'92%' },
  sheetHandle:  { width:40, height:4, backgroundColor: theme.colors.border, borderRadius:2, alignSelf:'center', marginBottom:16 },
  sheetHeader:  { flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:16 },
  sheetTitle:   { color: theme.colors.text, fontSize:18, fontWeight:'800' },
  searchBar:    { flexDirection:'row', alignItems:'center', backgroundColor: theme.colors.background, borderRadius:12, borderWidth:1, borderColor: theme.colors.border, paddingHorizontal:14, paddingVertical:11, gap:10, marginBottom:12 },
  searchInput:  { flex:1, color: theme.colors.text, fontSize:14 },
  resultsList:  { backgroundColor: theme.colors.background, borderRadius:14, borderWidth:1, borderColor: theme.colors.border, marginBottom:8, overflow:'hidden' },
  resultItem:   { flexDirection:'row', alignItems:'center', padding:12, borderBottomWidth:1, borderBottomColor: theme.colors.surface },
  resultName:   { color: theme.colors.text, fontSize:14, fontWeight:'600' },
  resultMeta:   { color: theme.colors.textTertiary, fontSize:11, marginTop:2 },
  ...
  orTxt:        { color: theme.colors.textTertiary, fontSize:11 },
  inputGroup:   { marginBottom:14 },
  inputLabel:   { color: theme.colors.textSecondary, fontSize:12, fontWeight:'600', marginBottom:6 },
  input:        { backgroundColor: theme.colors.background, color: theme.colors.text, borderRadius:12, paddingHorizontal:14, paddingVertical:12, borderWidth:1, borderColor: theme.colors.border, fontSize:14 },
  unitBadge:    { backgroundColor: theme.colors.background, borderRadius:12, paddingHorizontal:14, paddingVertical:12, borderWidth:1, borderColor: theme.colors.border, minWidth:55, alignItems:'center' },
  unitTxt:      { color: theme.colors.text, fontWeight:'700' },
  macroGrid:    { flexDirection:'row', flexWrap:'wrap', gap:10, marginTop:8, marginBottom:20 },
  macroCell:    { width:'47%', backgroundColor: theme.colors.background, borderRadius:14, borderWidth:1, borderColor: theme.colors.border, padding:12, alignItems:'center' },
  macroCellLabel:{ fontSize:11, fontWeight:'700', marginBottom:6 },
  macroCellInput:{ color: theme.colors.text, fontSize:22, fontWeight:'800', textAlign:'center', width:'100%' },
  macroCellUnit: { color: theme.colors.textTertiary, fontSize:11, marginTop:4 },
  saveBtn:      { flexDirection:'row', alignItems:'center', justifyContent:'center', gap:10, paddingVertical:16, borderRadius:16 },
  saveBtnTxt:   { color: theme.colors.text, fontSize:16, fontWeight:'800' },
});
